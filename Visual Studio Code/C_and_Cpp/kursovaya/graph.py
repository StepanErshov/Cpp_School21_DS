...
with open('C:/projects/Visual Studio Code/C_and_Cpp/kursovaya/rezult_for_Vi.txt', 'r') as file:
    Vc = [float(line.strip()) for line in file]
#          ↓вот здесь надо ввести свой путь до файла в каждом with open и всё↓   вот здесь ничего не менять    
with open('C:/projects/Visual Studio Code/C_and_Cpp/kursovaya/rezult.txt', 'r') as file:
    Vi = [float(line.strip()) for line in file]
#          ↓вот здесь надо ввести свой путь до файла в каждом with open и всё↓    вот здесь ничего не менять
with open('C:/projects/Visual Studio Code/C_and_Cpp/kursovaya/hw8.txt', 'r') as file:
...
plt.plot(Height, Va, label='Va', color = 'red', marker='o', markersize = 2)
plt.xlabel('Высота')
plt.ylabel('Скорость')
plt.grid()
plt.legend()
